...
/**
 * Save the database to JSON file asynchronously.
 * @param {Object} data - The database object to save.
 * @returns {Promise<void>}
 */
const saveDatabase = async (data) => {
    await writeFileAsync(dbFilePath, JSON.stringify(data, null, 2));
};
 ...
 * @param {Array} batchQueue - The batch queue with records to process.
...
const processBatchQueue = (batchQueue) => {
    ...
    for (const [id, record] of batchQueue) {
        const existingRecord = batchedData.get(id) || { version: 0 };
...
if (isMainThread) {
    ...
    // Worker thread logic
    const batchedData = processBatchQueue(workerData.batchQueue);
